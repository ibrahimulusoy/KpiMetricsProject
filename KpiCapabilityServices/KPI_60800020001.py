...
dfVisionTemp['Raw_Score'] = (dfVisionTemp['Screening']/dfVisionTemp['Enrollment'] * 100 * 0.99) + (dfVisionTemp['ReferralsReturned']/dfVisionTemp['Referrals'] * 100 * 0.01)
dfVision = round(dfVisionTemp.groupby('Campus_RowID')['Raw_Score'].mean(), 2)

# HEARING SCOPE
dfHearingTemp = df[df['ScreeningType'] == 'H']
dfHearingTemp['Raw_Score'] = (dfHearingTemp['Screening']/dfHearingTemp['Enrollment'] * 100 * 0.99) + (dfHearingTemp['ReferralsReturned']/dfHearingTemp['Referrals'] * 100 * 0.01)
dfHearing = round(dfHearingTemp.groupby('Campus_RowID')['Raw_Score'].mean(), 2)

# ACANTHOSIS SCOPE
dfAcanthosisTemp = df[df['ScreeningType'] == 'A']
dfAcanthosisTemp['Raw_Score'] = dfAcanthosisTemp['Screening']/dfAcanthosisTemp['Enrollment'] * 100
dfAcanthosis = round(dfAcanthosisTemp.groupby('Campus_RowID')['Raw_Score'].mean(), 2)

# SPINAL SCOPE
dfSpinalTemp = df[df['ScreeningType'] == 'S']
dfSpinalTemp['Raw_Score'] = (dfSpinalTemp['Screening']/dfSpinalTemp['Enrollment'] * 100 * 0.99) + (dfSpinalTemp['ReferralsReturned']/dfSpinalTemp['Referrals'] * 100 * 0.01)
